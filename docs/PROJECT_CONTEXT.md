...
- Manual candidate selection does not reserve recruitment capacity or complete a match. The job-service matching-seat reservation contract and the payment/work/chat confirmation Saga remain follow-up work. A worker cancellation atomically cancels a related `PENDING` matching before canceling the application.
- A worker can list and read only their own matching proposals and can idempotently decline a `PENDING` proposal. Declining records a `DECLINED` matching history while the application remains `APPLIED`; accepting remains unavailable until recruitment-seat reservation and the confirmation Saga exist.
