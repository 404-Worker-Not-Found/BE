...
- A shared-secret internal recruitment-completion endpoint rejects remaining `APPLIED` applications and cancels their `PENDING` matching proposals in one transaction. It stores status histories and `ApplicationRejected` Outbox events, is idempotent on repeated delivery, and refuses completion while a confirmation Saga has an unknown outcome or unfinished compensation. The job-service producer remains owned by the job domain.
