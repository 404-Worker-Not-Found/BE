...
지원 생성·취소, 지원자 조회, 점수 스냅샷과 Redis 대기열의 세부 기준은 [지원 도메인 설계](./matching-application-design.md)를 따른다.
...
| 상태 | 의미 | 허용 전이 |
| --- | --- | --- |
| `PAYMENT_PENDING` | 예치 완료 전 비공개 공고 | `OPEN`, `CLOSED` |
...
- 같은 알바생은 같은 공고에 지원 레코드를 하나만 만들 수 있으며 취소 후에도 재지원할 수 없다.
...
- 매칭 제안을 기다리는 동안에는 `APPLIED`를 유지하고, 매칭 확정 Saga가 완료될 때 `SELECTED`로 변경한다.
...
- 실제 운영의 에스크로·알바생 지급 방식 (개발용 PG는 토스페이먼츠 테스트 환경으로 확정)
- 취소, 실패, 노쇼의 환불 금액
- 실시간 전달 방식과 재연결 처리
- 알림 채널과 메시지 보관 기간
