...
`matching-service`에는 이 계약과 payment/work/chat 계약을 호출하는 클라이언트 및 확정 Saga가 구현되어 있다. `work-service`의 예정 근무 생성·취소와 `chat-service`의 채팅방 생성·종료 계약은 구현되어 있다. `payment-service`의 예치 잔액 기반 잠금·해제 계약도 구현되어 있다. 다만 검증된 PG 예치 반영과 `job-service`의 자리 예약 API는 아직 구현되지 않았으므로 실제 수락 호출은 의존 서비스가 준비되기 전까지 실패 닫힘 방식으로 종료된다. 외부 계약이 준비되지 않았는데도 `PENDING`을 확정 상태로 바꾸거나 임시 성공 응답을 사용하지 않는다.
