...
- 점주 조회는 지원 접수 승인에서 보존한 `owner_member_id`로 제한한다. 다른 점주의 지원은 조회하지 못하며, 상세 조회는 권한 오류로 처리한다.
...
- 지원자가 없는 공고의 현재 소유권을 matching-service 단독으로 확인할 수는 없다. 공고 담당 범위에 소유권 내부 계약이 생기면 빈 목록 요청도 해당 계약으로 검증한다.
