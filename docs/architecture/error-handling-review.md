...
중복 오류는 사전 검사에서 반환하며, auth 가입 저장의 `uk_auth_accounts_email`과 `uk_oauth_connections_provider_user` 위반도 각각 409 업무 오류로 변환한다. 변환은 저장 트랜잭션이 롤백되고 회원 보상 삭제를 시도한 뒤 `SignupService`에서 수행해 커밋 시점의 오류도 포함한다. 원인 예외와 보상 실패 정보는 보존한다. member의 DB 경쟁과 그 밖의 알 수 없는 제약 위반은 일괄적으로 업무 중복으로 간주하지 않는다.
