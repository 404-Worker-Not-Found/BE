...
public class MemberApplicationService {
	...
	public CreateMemberResponse createOwnerMember(CreateOwnerMemberRequest request) {
		...
		Member member =
				memberCommandService.createMember(
						request.name(), request.email(), request.phoneNumber(), request.role());
		Location storeLocation = createLocation(request.storeLocation());
		OwnerProfile ownerProfile =
				ownerCommandService.createOwnerProfile(
						member,
	...
	private WorkerProfile createWorkerProfile(Member member, CreateWorkerMemberRequest request) {
		...
		WorkerProfile workerProfile =
				workerCommandService.createWorkerProfile(
						member,
		...
		request
				...
				.forEach(
						availableTime ->
								workerCommandService.addAvailableTime(
										workerProfile,
	...
	private List<String> toPreferredBusinessTypes(List<WorkerPreferredBusinessType> businessTypes) {
		return businessTypes.stream().map(WorkerPreferredBusinessType::getBusinessType).toList();
	}

	private List<WorkerAvailableTimeResponse> toAvailableTimes(
			List<WorkerAvailableTime> availableTimes) {
		return availableTimes.stream().map(this::toAvailableTimeResponse).toList();
	}
